...
public class VersionIgnore_Updater {
    ...
    private VersionIgnore_Updater.UpdateResult result = VersionIgnore_Updater.UpdateResult.SUCCESS; // Used for determining the outcome of the update process
     ...
     */
    public VersionIgnore_Updater(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
        this.plugin = plugin;
        this.type = type;
        ...
        try {
            this.url = new URL(VersionIgnore_Updater.HOST + VersionIgnore_Updater.QUERY + id);
     ...
     * @see UpdateResult
     */
    public VersionIgnore_Updater.UpdateResult getResult() {
        this.waitForThread();
        return this.result;
    }
    ...
    private void saveFile(File folder, String file, String link) {
        ...
        try {
            ...
            final byte[] data = new byte[VersionIgnore_Updater.BYTE_SIZE];
            int count;
            if (this.announce) {
                this.plugin.getLogger().info("About to download a new update: " + this.versionName);
            }
            long downloaded = 0;
            while ((count = in.read(data, 0, VersionIgnore_Updater.BYTE_SIZE)) != -1) {
                downloaded += count;
            ...
            this.result = VersionIgnore_Updater.UpdateResult.FAIL_DOWNLOAD;
        } finally {
            try {
                if (in != null) {
                    in.close();
        ...
        try {
            ...
            while (e.hasMoreElements()) {
                ...
                if (entry.isDirectory()) {
                    ...
                    int b;
                    final byte buffer[] = new byte[VersionIgnore_Updater.BYTE_SIZE];
                    final FileOutputStream fos = new FileOutputStream(destinationFilePath);
                    final BufferedOutputStream bos = new BufferedOutputStream(fos, VersionIgnore_Updater.BYTE_SIZE);
                    while ((b = bis.read(buffer, 0, VersionIgnore_Updater.BYTE_SIZE)) != -1) {
        ...
        if (this.type != UpdateType.NO_VERSION_CHECK) {
            ...
            if (title.split(delimiter).length == 2) {
                ...
                if (this.hasTag(localVersion) || !this.shouldUpdate(localVersion, remoteVersion)) {
                    ...
                    this.result = VersionIgnore_Updater.UpdateResult.NO_UPDATE;
                    return false;
                }
            } else {
                ...
                this.result = VersionIgnore_Updater.UpdateResult.FAIL_NOVERSION;
                return false;
            }
        }
        return true;
    ...
    private boolean hasTag(String version) {
        for (final String string : VersionIgnore_Updater.NO_UPDATE_TAG) {
    ...
    private boolean read() {
        try {
            ...
            conn.addRequestProperty("User-Agent", VersionIgnore_Updater.USER_AGENT);
            ...
            this.versionName = (String) ((JSONObject) array.get(array.size() - 1)).get(VersionIgnore_Updater.TITLE_VALUE);
            this.versionLink = (String) ((JSONObject) array.get(array.size() - 1)).get(VersionIgnore_Updater.LINK_VALUE);
            this.versionType = (String) ((JSONObject) array.get(array.size() - 1)).get(VersionIgnore_Updater.TYPE_VALUE);
            this.versionGameVersion = (String) ((JSONObject) array.get(array.size() - 1)).get(VersionIgnore_Updater.VERSION_VALUE);
    ...
    private class UpdateRunnable implements Runnable {
        ...
        @Override
        public void run() {
            if (VersionIgnore_Updater.this.url != null) {
                // Obtain the results of the project's file feed
                if (VersionIgnore_Updater.this.read()) {
                    if (VersionIgnore_Updater.this.versionCheck(VersionIgnore_Updater.this.versionName)) {
                        if ((VersionIgnore_Updater.this.versionLink != null) && (VersionIgnore_Updater.this.type != UpdateType.NO_DOWNLOAD)) {
                            String name = VersionIgnore_Updater.this.file.getName();
                            // If it's a zip file, it shouldn't be downloaded as the plugin's name
                            if (VersionIgnore_Updater.this.versionLink.endsWith(".zip")) {
                                final String[] split = VersionIgnore_Updater.this.versionLink.split("/");
                                name = split[split.length - 1];
                            }
                            VersionIgnore_Updater.this.saveFile(new File(VersionIgnore_Updater.this.plugin.getDataFolder().getParent(), VersionIgnore_Updater.this.updateFolder), name, VersionIgnore_Updater.this.versionLink);
                        } else {
                            VersionIgnore_Updater.this.result = UpdateResult.UPDATE_AVAILABLE;
                        }
                    }
                }
            }
